...
**Fonts:** **ABC Arizona Flare** (Dinamo; 2 static weights, display/temple voice) + **IBM Plex Mono** (machine voice). Loaded via `expo-font` config plugin (embedded at build, no FOUT). Licensing: Dinamo **App/Game license** required before any store build ships the font (trial fonts for local mockups only); **Web license** if the marketing site uses live-text Arizona (or render it into images to skip it). Optional: variable superfamily instead of statics unlocks sans↔serif axis-morph (RN Skia variable-axis if workable, else stepped instances) — nice-to-have only. Fallback if licensing stalls near deadline (a real plan B, not a shrug): **Marcellus + Cinzel** (both Google Fonts/OFL) splitting the temple voice — Cinzel for carved-caps ritual moments, Marcellus for mixed-case display (single weight; hierarchy via size/spacing). Ship dates beat fonts.
