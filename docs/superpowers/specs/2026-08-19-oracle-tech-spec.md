...
**Fonts (launch):** **Marcellus + Cinzel** (Google Fonts/OFL) as the temple voice — Cinzel for carved-caps ritual moments, Marcellus for mixed-case display (single weight; hierarchy via size/spacing) — + **IBM Plex Mono** (machine voice). All OFL: committed to repo normally, loaded via `expo-font` config plugin (embedded at build, no FOUT), zero licensing overhead. **Upgrade path:** ABC Arizona Flare (Dinamo App/Game + Web licenses) swapped in via the same font slots if mid-Sept traction justifies it — trials live gitignored in `design/fonts/trial/` for mockups only and must never ship (see `design/fonts/README.md`).
