...
- **Subscription shield grant (decided at final review):** the webhook grants **+3 paid shields on `INITIAL_PURCHASE` and on each `RENEWAL`** of a plus product, capped at 5 total (`LEAST(shieldsRemaining + 3, 5)`) — no grant on `UNCANCELLATION` or `PRODUCT_CHANGE`, since neither starts a new billing period. This is the entitlement's functional benefit: design spec §5 rules "everyone gets one free auto-shield per month; Plus gets more," and the paywall's own copy promises it ("THE ORACLE FORGIVES ONCE A MONTH. PLUS FORGIVES MORE."). Before this fix Plus carried no shield benefit at all.
