...
- **§1.1 took three passes, and the last one is the lesson.** Naming the score in one rite left "0 OF 50 CALLS WRITTEN" still reading as a progress bar toward an unnamed thing, because nothing said what a CALL was — so the goal became three rites: V defines a call as an answer you sealed, VI says what fifty of them buy, VII says which ones count and that a partial day is not wasted. Then the real question surfaced on device: **"what is 50? aren't there only 5 questions?"** Fifty is a *lifetime* count printed on a screen whose every other surface says five — five questions, five numerals, seal all five — so read cold it looks like the same scale. Every appearance of the lifetime number now carries the rate that converts it: rite VI is "FIVE A DAY, SO TEN DAYS AT THE LEAST", the plaque gloss leads with "FIVE CALLS A DAY, FIFTY TO WRITE IT", and the reveal reads `ORACLE SCORE UNWRITTEN` / `0 OF 50 RATED CALLS · FIVE A DAY`. A copy-lint rule now fails any line containing FIFTY that does not state the rate beside it. ("At the least", not exactly: a voided question earns no brier, so the tenth day is a floor, never a promise.)
