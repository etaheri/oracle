...
const good = { iss: "https://appleid.apple.com", aud: "com.erikt.oracle", exp: NOW / 1000 + 600, sub: "apple-sub-1" };
...
describe("verifyAppleIdentityToken", () => {
  ...
  it("accepts a valid token and returns sub", async () => {
    ...
    const out = await verifyAppleIdentityToken(await sign(good), { audience: "com.erikt.oracle", fetchFn, nowMs: NOW });
    expect(out).toEqual({ sub: "apple-sub-1" });
  });
  it("rejects wrong audience, wrong issuer, expiry, unknown kid, bad signature, garbage", async () => {
    const { fetchFn, sign } = await makeApple();
    const opts = { audience: "com.erikt.oracle", fetchFn, nowMs: NOW };
  ...
  it("returns null when fetchFn rejects (network error)", async () => {
    ...
    const opts = { audience: "com.erikt.oracle", fetchFn: failFetch, nowMs: NOW };
  ...
  it("returns null when fetchFn returns invalid JSON", async () => {
    ...
    const opts = { audience: "com.erikt.oracle", fetchFn: badJsonFetch, nowMs: NOW };
    expect(await verifyAppleIdentityToken(await sign(good), opts)).toBeNull();
  });
  it("returns null when JWKS response has no keys array", async () => {
    ...
    const opts = { audience: "com.erikt.oracle", fetchFn: noKeysFetch, nowMs: NOW };
    expect(await verifyAppleIdentityToken(await sign(good), opts)).toBeNull();
  });
