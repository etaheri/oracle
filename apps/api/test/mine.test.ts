...
describe("GET /v1/round/today/mine", () => {
  it("returns only the caller's own predictions for the open round", async () => {
    ...
    expect(out.predictions).toEqual([{ question_id: qs[0]!.id, answer: true, confidence: 75, crowd_yes_pct_at_seal: 100, crowd_count_at_seal: 1, stake: null, doubled: false }]);
  });
  ...
  it("carries the crowd snapshot taken at the seal (design 2026-09-09 §4.1)", async () => {
    ...
    expect(out.predictions).toEqual([
      { question_id: qs[0]!.id, answer: true, confidence: 75, crowd_yes_pct_at_seal: 100, crowd_count_at_seal: 1, stake: null, doubled: false },
    ]);
  });
