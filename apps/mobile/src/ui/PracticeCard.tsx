...
import { Mono, Serif, Ritual } from "./Text";
...
export function PracticeCard({ onCompleted }: { onCompleted: () => void }) {
  ...
  return <View style={{ flex: 1, minHeight: 0, gap: space(3) }}>
    <CardStage>{height => <View>
      {receipt ? <CardChrome height={height} slot={1} title={revealed ? "PRACTICE RESULT" : "PRACTICE SEALED"} status="FICTIONAL · UNSCORED">
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1, justifyContent: "center", gap: space(4) }} contentInsetAdjustmentBehavior="never" alwaysBounceVertical={false}>
          <Mono size={11} style={{ textAlign: "center" }}>YOU CHOSE {receipt.answer ? "YES" : "NO"} AT {receipt.confidence}%</Mono>
          {revealed ? <PracticeResult prediction={receipt} previous={previous} /> : <>
            ...
            <Mono size={11} style={{ textAlign: "center" }}>In the daily game, you wait for the real outcome. Here, reveal a fictional result now.</Mono>
          </>}
        </ScrollView>
    ...
    <View style={{ minHeight: 48, gap: space(1), justifyContent: "center" }}>
      ...
      </> : receipt ? <Mono size={10} style={{ textAlign: "center" }}>PRACTICE ONLY · YOUR SCORE AND STREAK ARE UNCHANGED.</Mono> :
        <Mono size={10} style={{ textAlign: "center" }}>{previous ? "SAME FICTIONAL GAME. TRY A DIFFERENT CONFIDENCE OR SIDE." : "PULL TO ADJUST. RETURN TO CENTER TO CANCEL. RELEASE TO SEAL."}</Mono>}
    </View>
...
function PracticeResult({ prediction, previous }: { prediction: PracticePrediction; previous: PracticePrediction | null }) {
  ...
  return <>
    ...
    <Mono size={10} style={{ textAlign: "center" }}>FICTIONAL OUTCOME · YES</Mono>
    ...
    <Mono size={11} style={{ textAlign: "center" }}>
    ...
    {previous && <Mono size={11} style={{ textAlign: "center" }}>
    ...
    <Mono size={11} style={{ textAlign: "center" }}>In a real round, choose the confidence you actually believe. The outcome is not known when you seal.</Mono>
  </>;
}
